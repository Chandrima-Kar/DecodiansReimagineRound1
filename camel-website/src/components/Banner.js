...
import { IoMdColorPalette, IoIosArrowDropdown, IoIosArrowDropup } from "react-icons/io";
...
export const circularLink = [
  { id: "0", title: "Paints", icon: <MdOutlineFormatColorFill />, path: "/paints" },
  { id: "1", title: "Drawing Materials", icon: <IoBagHandle />, path: "/drawing-materials" },
          ...
          <div className="relative left-1/2 flex aspect-square border-[3px] border-accent rounded-full -translate-x-1/2 scale-75 md:scale-100 w-[20rem] sm:w-[35rem] lg:max-xl:w-[25rem]">
                ...
                <Link to="/global-search" className="flex items-center justify-center w-full h-full bg-background rounded-full">
                  <div className="relative flex items-center justify-center text-[3rem] md:text-7xl text-primary-text">
                    ...
                    <img
                      src={camel_img}
                      alt="camel_image"
                      className="absolute w-[17px] h-[17px] md:w-[25px] md:h-[25px] top-[0.7rem] left-[0.75rem] md:top-[1.20rem] md:left-[1.20rem]"
                    />
                  </div>
                </Link>
              </div>
            ...
            <ul>
              {circularLink.map((app, index) => (
                <li
                  key={app.id}
                  className="absolute top-0 left-1/2 h-1/2 origin-bottom -ml-[4rem] md:-ml-[4.6rem]"
                ...
                >
                  {app.title === "Community" ? (
                    ...
                    >
                      <div
                        className={`relative gap-x-1 -top-[1.6rem] flex items-center justify-center font-semibold border-2 border-accent rounded-xl pl-2 transition-all duration-500
                        w-[8rem] md:w-[10rem] h-[1.8rem] md:h-[2.2rem] text-[12px] md:text-[1rem] ${showDropdown ? "rounded-b-none bg-accent text-background" : "bg-background text-accent hover-accent2"}`}
                        style={{ transform: `rotate(-${index * 36}deg)` }}
                      >
                        <div>{app.title}</div>
                        <div>{app.icon}</div>
                      ...
                      <div
                        className={`absolute bg-background2 border border-accent text-center duration-500 transform w-[8rem] md:w-[10rem] -top-[0.5rem] right-[2.05rem] md:-top-[0.3rem] md:right-[2.4755rem] ${showDropdown ? "opacity-100 scale-100" : "opacity-0 scale-0"}`}
                        style={{ transform: `rotate(-${index * 36}deg)` }}
                      >
                        {dropDownContent.map((content, index) => (
                          <Link key={index} to={content.path} className="w-full h-full text-[12px] md:text-[1rem]">
                            <div className="flex flex-col font-semibold border border-accent py-1 bg-background2 hover:bg-accent text-primary-text hover:text-background hover-accent2">
                              {content.title}
                            </div>
                          </Link>
                        ))}
                    ...
                    <div>
                      <div className="relative -top-[1.6rem]" style={{ transform: `rotate(-${index * 36}deg)` }}>
                        <Link to={app.path} className="flex flex-row items-center justify-center">
                          <div className="gap-x-1 flex items-center justify-center font-semibold bg-background hover:bg-accent border-2 border-accent rounded-xl text-accent hover-accent2 transition-all duration-500 w-[8rem] md:w-[10rem] h-[1.8rem] md:h-[2.2rem] text-[11px] md:text-[1rem]">
                            <div>{app.title}</div>
                            <div>{app.icon}</div>
                          </div>
                        </Link>
